...
function getCount(appConfig: AppConfig[]) {
  ...
  appConfig.forEach(({ addType, allowNotification }) => {
    if (allowNotification === false) {
      disabledCount++
    }
    if (addType === 'auto') {
      autoCount++
    } else if (addType === 'external') {
      externalCount++
    } else {
      manualCount++
    }
  ...
  return { allCount, manualCount, autoCount, externalCount, disabledCount }
}
...
export default function addRankingFeedItem(props: {
  ...
  const { allCount, manualCount, autoCount, externalCount, disabledCount } =
    getCount(appConfig)
    ...
    (res, item) => {
      ...
      if (isInclude) {
        ...
        if (allowNotification === false) {
          ...
          res.disableApps.push({ id, name: appInfo.name, addType, addSource: item.addSource })
  ...
  const {
    ...
    externalCount: regionExternalCount,
        ...
        <table>
          ...
          <tbody>
            {disableApps.map((disableApp, index) => {
              const { id, name, addType, addSource } = disableApp
              return (
                <tr>
                  <td>
                    <b>{index + 1}</b>
